...
function Update-DeviceFunction {
    ...
    $DeviceFunctionMap = @{
        ...
        '-gw'         = 'GateWay'
        'print'       = 'Print Server'
        'radionode'   = 'Radio Node'
        'idrac'       = 'Remote Management Interface'
        'tc'          = 'Clock'
        '-fw'         = 'Network Security'
        'file'        = 'File Server'
        '-ups'        = 'UPS (Uninterruptible Power Supply) for network operations.'
        'techops'     = 'Technical Operations'
        'ae'          = 'Application Engine'
        'shore'       = 'ShoreTel'
        'bfxrly'      = 'Back-End Finance Relay'
        'crtxbkr'     = 'Credit Transaction Broker'
        'deploy'      = 'Deployment Server'
        'amosapp'     = 'Application Management and Optimization System Application'
        'amosrep'     = 'Application Management and Optimization System Application Reporting'
        'web'         = 'Web Server'
        'traka'       = 'Tracking Server'
        'sarg'        = 'SARG (Squid Analysis Report Generator).'
        'sargdb'      = 'Database server for SARG reports'
        'snmid'       = 'Sensor middleware device'
        'dir'         = 'Directory server'
        'visi'        = 'Visualization server'
        'foreman'     = 'Foreman'
        'awx'         = 'AWX server, an open-source web-based user interface for Ansible.'
        'build'       = 'Build server for continuous integration/continuous deployment (CI/CD)'
        'nfs'         = 'Network File System server'
        'rkenginx'    = 'Nginx server related to RKE (Rancher Kubernetes Engine)'
        'vaultnginx'  = 'Nginx server for a Vault application (likely HashiCorp Vault)'
        'convlt'      = 'Conversion or Virtualization processes'
        'redis'       = 'Redis in-memory data structure store instances'
        'rkem'        = 'RKE management servers for Rancher'
        'rkew'        = 'RKE worker nodes'
        'vrlog'       = 'Logging servers for VR'
        'vrops'       = 'vRealize Operations servers for monitoring'
        'iface'       = 'Interface server'
        'pmm'         = 'Performance Monitoring and Management'
        'ohcweb'      = 'Web server for an OHC (Onboarding Health Check) application'
        'sch'         = 'Scheduler server'
        'pos'         = 'Point of Sale system'
        'poscaps'     = 'Point of Sale capabilities'
        'posrpt'      = 'Point of Sale reporting server'
        'fms'         = 'File Management System server'
        'ohc'         = 'OHC payment processes'
        'rke-ingress' = 'Ingress controller for RKE'
        'rancher'     = 'General Rancher management server'
        'vx'          = 'Virtual machines services'
        'vxmgr'       = 'Management server for VX instances'
        'vrlcm'       = 'vRealize Lifecycle Manager server'
        'vridmgr'     = 'VR ID Manager or related service'
        'vxvcsa'      = 'Virtual Cloud Services Appliance.'
...
function Update-Department {
    ...
    <#
    .SYNOPSIS
        Updates the 'Department' custom property of a node based on its Caption.
    
    .DESCRIPTION
        This function checks the node's 'Caption' property for specific keywords and maps them to predefined departments.
        If a match is found, the function updates the 'Department' custom property accordingly. If no matches are found, no changes are made.
    
    .PARAMETER Node
        A PSCustomObject representing a node. The object should contain the 'Caption' and 'Department' custom properties.
    
    .EXAMPLE
        PS> $updatedDeviceFunction = Update-Department -Node $node
    
        Checks the node's 'Caption' for matching keywords and updates the 'Department' custom property.
    
    .NOTES
        Author: Ryan Woolsey
        Last Edit: 9-20-2024
        Version: 1.1
        Keywords: SolarWinds, Department, Node Properties, OrionSDK
    
    .LINK
        https://github.com/solarwinds/OrionSDK/wiki/PowerShell
    
    .INPUTS
        [PSCustomObject] - A node object with relevant properties like Caption and Department.
    
    .OUTPUTS
        [string] - Returns the updated or unchanged department string.
    #>

    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true, 
            HelpMessage = "Provide the node object which contains the Caption and Department properties.")]
        [PSCustomObject]$Node
    )
    ...
    $DepartmentUpdated = $false  # Track if an update is made
    ...
    foreach ($key in $DepartmentMap.Keys) {
        ...
        # Check if the Caption contains the keyword
        if ($Node.Caption -match $key) {
            $newDepartment = $DepartmentMap[$key]
            Write-Verbose "[UPDATE] $($Node.Caption): Department updated to '$newDepartment' based on keyword '$key'."
    ...
    if (-not $DepartmentUpdated) {
        Write-Verbose "[NO CHANGE] $($Node.Caption): Department is already set to '$($Node.Department)' or no matching keyword found."
    }
...
function Build-NodeUpdateList {
    ...
    foreach ($Node in $Nodes) {
        ...
        $newDepartment = Update-Department -Node $Node
        ...
        if ($newDepartment -ne $oldDepartment) {
            Write-Host "[CHANGE] $($Node.Caption): Department changed from '$oldDepartment' to '$newDepartment'" -ForegroundColor Cyan
            $properties['Department'] = $newDepartment
        }
