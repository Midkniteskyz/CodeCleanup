function Update-OrionCustomPropertyWithValues {
    param (
        ...
        [string[]]$NewValues               # New values to add to the custom property
    )
    ...
    foreach ($Hostname in $Hostnames) {
        try {
            ...
            if ($existingProperty) {
                Write-Host "Custom property '$PropertyName' exists on $Hostname. Updating values..."
...
$newValues = @('NewFunction1', 'NewFunction2', 'NewFunction3')
