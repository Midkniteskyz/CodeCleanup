function Get-CustomProperty {
    ...
    Begin {
        ...
        # Assuming $PropertyName is an array of property names
        $whereClause = ""
        if ($PropertyName) {
            if ($PropertyName -like '*`*') {
                # Handle case where PropertyName contains a wildcard
                $whereClause = "`nWHERE " + (($PropertyName.Replace('*', '%') | ForEach-Object { "CP.Field LIKE '$_'" }) -join " OR ")
                Write-Host "Wildcard detected. WHERE clause: $whereClause"
            } else {
                # Handle exact matches
                $whereClause = "`nWHERE " + (($PropertyName | ForEach-Object { "`nCP.Field = '$_'" }) -join " OR ")
                Write-Host "No wildcard detected. WHERE clause: $whereClause"
            }
        }
